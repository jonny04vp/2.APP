...
st.markdown("""
Esta simulación representa dos cilindros uno más grande que el otro. En uno de ellos está enrollado un cable que sostiene un bloque de una determinada masa.
""")
...
st.markdown("""
Mediante esta aplicación el usuario podrá calcular el tiempo que se demora en caer la caja desde una determinada altura, además de las velocidades de los discos y del bloque.
""")
st.markdown("""
Mediante los gráficos interactivos se puede observar cómo disminuye la energía potencial mientras aumenta la energía cinética.
""")
...
st.sidebar.header("Desenrollamiento del cable")
origen_disco = st.sidebar.radio(
    "¿Desde qué disco se desenrolla el cable?",
    ("Disco interno (r)", "Disco externo (R)")
)
r_efectivo = r if origen_disco == "Disco interno (r)" else R
...
a = 9.81 / (1 + I_total / (m * r_efectivo**2))
...
st.markdown(f"**Momento de inercia total (I₁ + I₂):** {I_total:.4f} kg·m²")
st.markdown(f"**Tiempo total de caída:** {t_total:.2f} s")
...
def generar_pdf():
    ...
    pdf.cell(0, 8, f"Cable desenrollado desde: {'disco interno (r)' if r_efectivo == r else 'disco externo (R)'}", 0, 1)

    pdf.ln(5)
    pdf.cell(0, 10, "Resultados finales:", 0, 1)
    pdf.cell(0, 8, f"Momento de inercia total: {I_total:.4f} kg·m²", 0, 1)
...
st.markdown("""
Para comprobar que esta aplicación calcule correctamente cada variable se tomó el ejercicio 9.89 del libro  
**Física Universitaria**  
YOUNG • FREEDMAN  
Volumen 1 - Sears & Zemansky
""")

st.markdown("[Consulta el libro aquí (PDF)](https://blog.espol.edu.ec/srpinarg/files/2014/05/Fisica-Universitaria-Sears-Zemansky-12ava-Edicion-Vol1.pdf)")
